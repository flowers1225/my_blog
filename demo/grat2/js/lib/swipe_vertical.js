...
function SwipeVertical(container, options) {
    ...
    move: function(event) {
      ...
      if (!isScrolling) {
        ...
        // increase resistance if first or last slide
          ...
          if(direction && ($(event.srcElement).parents('.locked').length > 0 || $(event.srcElement).hasClass('locked'))){
              //todo:回调提示
            return false;
          }
  ...
  return {
    ...
    cancelSwipeUp: function(callback){
        // removed event listeners
        if (browser.addEventListener) {
